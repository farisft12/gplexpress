...
@section('title', 'Manajemen User - GPL Express')
...
<div>
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 lg:mb-8">
        ...
        @if(Auth::user()->isOwner())
            <a href="{{ route('admin.users.create') }}" class="bg-[#F4C430] text-white px-4 py-2.5 lg:px-6 lg:py-3 rounded-lg font-semibold hover:bg-[#E6B020] transition-colors text-sm lg:text-base whitespace-nowrap">
                + Tambah User
            </a>
        @endif
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm p-4 lg:p-6 mb-6">
        <form method="GET" action="{{ route('admin.users.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div>
                ...
                <select name="role" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#F4C430] focus:border-transparent outline-none">
                    ...
                    @else
                        {{-- Manager can only see admin and kurir --}}
                        <option value="admin" {{ request('role') === 'admin' ? 'selected' : '' }}>Admin</option>
                        <option value="kurir" {{ request('role') === 'kurir' ? 'selected' : '' }}>Kurir</option>
                    @endif
                </select>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($users as $user)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="px-2 py-1 text-xs font-semibold rounded-full {{ $user->role === 'admin' ? 'bg-[#F4C430]/10 text-[#F4C430]' : ($user->role === 'manager' ? 'bg-purple-100 text-purple-700' : ($user->role === 'kurir' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-700')) }}">
                                    {{ ucfirst($user->role) }}
                                </span>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                @if(Auth::user()->isOwner())
                                    <a href="{{ route('admin.users.edit', $user) }}" class="text-[#F4C430] hover:text-[#E6B020] mr-3">Edit</a>
                                    @if($user->id !== auth()->id())
                                    ...
                                    @endif
                                @else
                                    <a href="{{ route('admin.users.show', $user) }}" class="text-[#F4C430] hover:text-[#E6B020]">Detail</a>
                                @endif
                            </td>
                        </tr>
                    @empty
