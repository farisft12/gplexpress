...
@section('title', 'Detail Paket Non-COD - GPL Express')
@section('page-title', 'Detail Paket Non-COD')

@section('content')
<div>
    <div class="mb-6 lg:mb-8">
        <a href="{{ route('admin.reports.non-cod', request()->except(['date', 'year', 'week', 'month', 'group_by'])) }}" 
           class="text-[#F4C430] hover:underline mb-4 inline-block text-sm lg:text-base">← Kembali ke Laporan Non-COD</a>
        <h1 class="text-2xl lg:text-3xl font-bold text-gray-900">Detail Paket Non-COD</h1>
    ...
    <div class="bg-white rounded-xl shadow-md p-6 mb-6 border-l-4 border-blue-500">
        ...
        <p class="text-3xl font-bold text-gray-900">{{ number_format($total, 0, ',', '.') }}</p>
    </div>
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if($shipment->shipping_cost)
                                    <div class="text-sm font-semibold text-gray-900">Rp {{ number_format($shipment->shipping_cost, 0, ',', '.') }}</div>
                                @else
                                    <div class="text-sm text-gray-400">Belum dihitung</div>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
