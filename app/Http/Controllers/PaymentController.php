...
class PaymentController extends Controller
{
    ...
    public function showPaymentForm($shipment = null)
    {
        ...
        $shipmentParam = $shipment ?? request()->route('shipment') ?? request()->route('shipmentId');
        ...
        if ($shipmentParam instanceof \App\Models\Shipment) {
            ...
            $shipment = \App\Models\Shipment::withoutGlobalScope(\App\Models\Scopes\BranchScope::class)
                ->findOrFail($shipmentParam);
        }
        
        // Check authorization - allow admin and manager from origin or destination branch
        $user = auth()->user();
        if (!$user->isOwner()) {
            if ($user->branch_id) {
                ...
                if ($shipment->branch_id !== $user->branch_id && $shipment->destination_branch_id !== $user->branch_id) {
                    abort(403, 'Anda tidak memiliki akses untuk melihat halaman pembayaran paket ini.');
                }
            }
        }
        ...
        if (!$user->isOwner()) {
            if ($user->branch_id) {
                ...
                if ($shipment->branch_id !== $user->branch_id && $shipment->destination_branch_id !== $user->branch_id) {
                    ...
                    abort(403, 'Anda tidak memiliki akses untuk memproses pembayaran paket ini.');
                }
            }
        }
        ...
        if (!$user->isOwner()) {
            if ($user->branch_id) {
                ...
                if ($shipment->branch_id !== $user->branch_id && $shipment->destination_branch_id !== $user->branch_id) {
                    return response()->json([
                        'success' => false,
                        'message' => 'Anda tidak memiliki akses untuk melihat status pembayaran paket ini.'
                    ], 403);
                }
            }
        }
